...
func newService(src *fakeSource, st *memoryStore) *Service {
	...
	return NewService(src, st, zap.NewNop(), noopMetrics{}, func() time.Time { return now }, 8*24*time.Hour, 72*time.Hour, 96*time.Hour, false, nil)
}
...
func TestServiceSendsFirstReminderOnDayThree(t *testing.T) {
	...
	svc := NewService(src, st, zap.NewNop(), noopMetrics{}, func() time.Time { return now }, 8*24*time.Hour, 72*time.Hour, 96*time.Hour, false, nil)
...
func TestServiceDryRunDoesNotCallSendMessage(t *testing.T) {
	...
	svc := NewService(src, st, zap.NewNop(), noopMetrics{}, func() time.Time { return now }, 8*24*time.Hour, 72*time.Hour, 96*time.Hour, true, nil)
